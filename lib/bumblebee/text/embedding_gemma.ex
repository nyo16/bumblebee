defmodule Bumblebee.Text.EmbeddingGemma do
  ...
  @moduledoc """
  EmbeddingGemma model family.

  EmbeddingGemma is built on the Gemma 3 transformers backbone, but modified to use 
  bi-directional attention instead of causal (one-way) attention. This effectively 
  turns the architecture from a decoder into an encoder, making it suitable for 
  embedding generation rather than text generation.

  ## Architectures

    * `:base` - plain EmbeddingGemma without any head on top

    * `:for_text_embedding` - EmbeddingGemma with mean pooling applied to
  ...
  def model(%__MODULE__{architecture: :for_text_embedding} = spec) do
    inputs = inputs(spec)

    outputs = core(inputs, spec)

    # Apply mean pooling for text embeddings
    pooled_output =
      Layers.if_present inputs["attention_mask"] do
        Axon.layer(
          fn hidden_state, attention_mask, _opts ->
            ...
            attention_mask_expanded = Nx.new_axis(attention_mask, -1)
        ...
        # If no attention mask, just compute mean across sequence length
        Axon.layer(
  ...
  defp encoder(
    ...
    Layers.Transformer.blocks(hidden_state,
      ...
      causal: false, # This is the key difference - bidirectional attention!
      rotary_embedding: [
  ...
  defimpl Bumblebee.HuggingFace.Transformers.Model do
    def params_mapping(_spec) do
      %{
        ...
        "encoder.blocks.{n}.self_attention.query" => "model.layers.{n}.self_attn.q_proj",
        "encoder.blocks.{n}.self_attention.key" => "model.layers.{n}.self_attn.k_proj",
        "encoder.blocks.{n}.self_attention.value" => "model.layers.{n}.self_attn.v_proj",
        "encoder.blocks.{n}.self_attention.output" => "model.layers.{n}.self_attn.o_proj",
        "encoder.blocks.{n}.self_attention_norm" => "model.layers.{n}.input_layernorm",
        "encoder.blocks.{n}.self_attention.rotary_embedding" =>
          "model.layers.{n}.self_attn.rotary_emb",
        "encoder.blocks.{n}.ffn.gate" => "model.layers.{n}.mlp.gate_proj",
        "encoder.blocks.{n}.ffn.intermediate" => "model.layers.{n}.mlp.up_proj",
        "encoder.blocks.{n}.ffn.output" => "model.layers.{n}.mlp.down_proj",
        "encoder.blocks.{n}.output_norm" => "model.layers.{n}.post_attention_layernorm",
