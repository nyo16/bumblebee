# 4-GPU Tensor Parallel Text Generation with KV Cache
#
...
#
# Usage:
#   XLA_FLAGS="--xla_force_host_platform_device_count=4" mix run examples/tp_4gpu_generate_kvcache.exs
#   # Or on real GPUs:
...
IO.puts("4-GPU Tensor Parallel Text Generation with KV Cache")
IO.puts("=" |> String.duplicate(70))
...
# Model size - will be loaded from TinyLlama
...
{:ok, %{params: params, spec: spec}} = Bumblebee.load_model({:hf, "TinyLlama/TinyLlama-1.1B-intermediate-step-1431k-3T"})
{:ok, tokenizer} = Bumblebee.load_tokenizer({:hf, "TinyLlama/TinyLlama-1.1B-intermediate-step-1431k-3T"})
...
embed_tokens = param_data["embedder"]["kernel"]
final_norm_weight = param_data["decoder"]["final_norm"]["scale"]
lm_head_kernel = param_data["language_modeling_head"]["kernel"]

# Extract and shard layer parameters
layer_params = for layer_idx <- 0..(num_layers - 1) do
  layer_prefix = "decoder.blocks.#{layer_idx}"

  sa_norm = param_data["#{layer_prefix}.self_attention_norm"]["scale"]
  ffn_norm = param_data["#{layer_prefix}.ffn_norm"]["scale"]
...
# TODO: Decode phase will be implemented next
IO.puts("\n  Note: Decode phase (with KV cache reuse) will be implemented next")
...
✓ Prefill phase working
  - Processes prompt in single forward pass
  - Outputs logits + K/V cache for all layers
  - Cache shape: [batch, local_kv_heads, seq_len, head_dim] per layer per GPU

⧗ Decode phase (next step)
  - Will accept single token + previous K/V cache
  - Concatenate new K/V to cache
  - Only compute attention for new token
  - Much faster than recomputing full sequence

Performance gain with KV cache:
  - Without: O(n²) - recompute all tokens each step
  - With: O(n) - only compute new token

Next: Implement decode phase SPMD and autoregressive generation loop.
""")
